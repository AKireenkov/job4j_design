...
public class ListUtilsTest {
    ...
    public void whenAddAfter() {
        ...
        ListUtils.addAfter(input, 2, 9);
        assertThat(input, is(Arrays.asList(0, 1, 2, 9, 3, 4)));
    }

    @Test
    public void whenRemoveIf() {
        List<Integer> input = new ArrayList<>(Arrays.asList(1, 2, 3, 4, 0));
        Predicate<Integer> filter = f -> f > 2;
        ListUtils.removeIf(input, filter);
        assertThat(input, is(Arrays.asList(1, 2, 0)));
    }

    @Test
    public void whenReplaceIf() {
        List<Integer> input = new ArrayList<>(Arrays.asList(0, 1, 2, 3, 4));
        Predicate<Integer> filter = f -> f == 1;
    ...
    public void whenRemoveAll() {
        List<Integer> input = new ArrayList<>(Arrays.asList(0, 1, 2, 3, 4));
        List<Integer> elements = new ArrayList<>(Arrays.asList(0, 2, 4));
